...
**离线模型的哈希是我们自己算的。** Argos 上游的包索引里没有任何校验和，
`localmodel.rs` 里那 11 个 sha256 是把每个包完整下下来算出来的。加语言方向时
必须同样实算——**不能编、不能猜**，写错的后果是那个方向永远装不上，而且报错
看起来像网络问题。算法：`curl -fsSL <url> | shasum -a 256`，同时核对字节数和
注册表里的 `bytes` 对不对得上（对不上说明下载被截断，算出来的哈希是错的）。
校验失败时会**删掉** .partial，和其它错误路径相反——残档保留是为了续传，
但内容已经错的文件续不出正确结果，只会让用户点几次失败几次。

**截图必须等遮罩真的从屏幕上消失。** `region.rs` 里 `hide()` 之后 sleep 了
120ms 才截图——`hide()` 只是发出请求，合成器把窗口从画面上抹掉还要一两帧。
不等的话截到的是那层灰罩子，而且是「有时候正常有时候发灰」的随机现象。

**遮罩的选区坐标是 CSS 像素，截图要的是全局物理像素。** 中间差一个显示器缩放
系数和一个显示器原点。少乘缩放，在 150% 的机器上截出来的区域会比框的小三分之一，
